...
An agent implementing an ExecPlan must create a new branch for each ExecPlan. A fix must be prefixed with "fix/", a feature must be prefixed with "feature/". The agent should create multiple commits during its work if its sensible. The agent should push the branch and open a PR after the plan is completed. 
